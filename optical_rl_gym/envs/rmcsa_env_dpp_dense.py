...
class RMCSADPPDenseEnv(RMCSAEnv):
    ...
    def __init__(
        ...
        default_actions_shape = self._get_default_actions_shape()
        ...
        # Alteração: Utilizar a extensão .log para indicar que é um arquivo de log
        handler = logging.FileHandler(
            f"rmcsa_env_dpp_dense_{seed}_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.log"
        )
    ...
    def _validate_action_parameters(self, action: List[int]) -> bool:
        """
        Valida se os parâmetros da ação estão dentro dos limites permitidos.

        :param action: Lista de inteiros representando a ação.
        :return: True se a ação é válida; caso contrário, False.
        """
    ...
    def _get_default_actions_shape(self):
        return (
            # working path
            self.k_paths + 1,
    ...
    def close(self):
        """Fecha os recursos abertos pelo ambiente, como o arquivo CSV e os handlers do logger."""
        if hasattr(self, 'csv_file') and self.csv_file:
            try:
                self.csv_file.close()
            except Exception as e:
                self.logger.error("Erro ao fechar o arquivo CSV: %s", e)
        # Fecha e remove cada handler do logger
        for handler in self.logger.handlers:
            handler.close()
            self.logger.removeHandler(handler)
    ...
    def calculate_modulation_error(self, path: Path, modulation: Modulation) -> float:
        """
        Calcula um erro simples para a modulação escolhida, comparando sua eficiência espectral com a do
        formato ideal para o comprimento do caminho.
        Essa função pode ser refinada conforme modelos mais complexos na literatura.
        """
        # Suponha que get_best_modulation_format retorna a modulação ideal para o dado comprimento de caminho.
        ideal_mod = get_best_modulation_format(path.length, self.modulation_formats)
        # Erro definido como a diferença absoluta das eficiências espectrais.
        error = abs(modulation.spectral_efficiency - ideal_mod.spectral_efficiency)
        return error

    def reward(self):
        """
        Função de recompensa para o ambiente de redes ópticas com proteção DPP, inspirada em abordagens 
        da literatura para Routing and Spectrum Assignment (ex.: Chakraborty et al. (2018)) e os fundamentos 
        em RL de Sutton & Barto (2018).

        Aspectos considerados:
        1. Serviço aceito: bônus se os caminhos de trabalho e backup forem provisionados, penalidade caso contrário.
        2. Disjointness: bônus se os caminhos forem disjuntos, penalidade se não forem.
        3. Eficiência Espectral: penalidade proporcional à utilização dos recursos (indicador de congestão).
        4. Aproximação da modulação ideal: penalização proporcional ao erro de modulação para cada caminho.
        5. Throughput: bônus adicional se o throughput global for alto.
        ...
        # 1. Serviço aceito (provisionamento de ambos os caminhos)
        if self.current_service.accepted:
            reward += 20.0  # bônus base para a aceitação do serviço
        else:
            return -20.0  # penalidade forte para serviço rejeitado

        # 2. Caminhos disjuntos
        if self.current_service.is_disjoint:
            reward += 10.0
        else:
            reward -= 10.0

        # 3. Penalização pela utilização dos recursos:
        # Quanto maior a fração de slots utilizados, maior a penalidade.
        used_slots = np.sum(self.topology.graph["available_slots"] == 0)
        total_slots = self.topology.graph["available_slots"].size
        spectrum_utilization = used_slots / total_slots
        reward -= spectrum_utilization * 5.0  # escala ajustável

        # 4. Penalização para modulação subótima:
        working_mod_error = self.calculate_modulation_error(
            self.current_service.working_path, self.current_service.working_modulation
        )
        backup_mod_error = self.calculate_modulation_error(
            self.current_service.backup_path, self.current_service.backup_modulation
        )
        reward -= (working_mod_error + backup_mod_error) * 2.0  # fator de penalização

        # 5. Bônus para throughput global:
        reward += self.throughput * 0.15
    ...
    #     # Recompensa/penaliza caminhos disjuntos
    #     reward += 4.0 if self.current_service.is_disjoint else -4.0
        
    #     # Alocação de slots
    #     reward += 2.0 if self.current_service.slots_assigned else -2
    #     reward += 2.0 if self.current_service.slots_assigned_backup else -2

    #     # Modulação aceitável
    #     reward += 3.0 if self.current_service.is_modulation_acceptable else -3.0

    #     # Aceitação do serviço + modulação
    #     if self.current_service.accepted:
    #         reward += 10.0
    #         working_mod_fitness = self.calculate_modulation_fitness(
    #             self.current_service.working_path,
    #             self.current_service.working_modulation
    #         )
    #         backup_mod_fitness = self.calculate_modulation_fitness(
    #             self.current_service.backup_path,
    #             self.current_service.backup_modulation
    #         )
    #         reward -= (working_mod_fitness + backup_mod_fitness) * 0.5
    #     else:
    #         reward -= 10.0

    #     # Utilização de recursos
    #     utilized_slots = np.sum(self.topology.graph["available_slots"] == 0) # slots utilizados na topologia
    #     total_slots = self.topology.graph["available_slots"].size # total de slots na topologia
    #     reward -= (utilized_slots / total_slots) * 0.1

    #     # Aumenta a influência do throughput
    #     reward += self.throughput * 0.1

    #     return reward

def shortest_available_path_best_modulation_first_core_first_fit(env: RMCSADPPDenseEnv):
