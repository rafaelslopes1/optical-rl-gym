...
class RMSADPPEnv(OpticalNetworkEnv):
    ...
    def step(self, action: [int]):  # We can refuse the service if one of the two is not working
      working_path, initial_slot_working, backup_path, initial_slot_backup = action[0], action[1], action[2], action[
          3]
      self.actions_output[
          working_path, initial_slot_working, backup_path, initial_slot_backup] += 1  # Will need to modify the actions_output
